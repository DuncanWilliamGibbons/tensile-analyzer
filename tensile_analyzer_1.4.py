"""
Tensile Test Data Analysis Program
Compliant with ASTM E8 and E112 standards

Author: Created for tensile test analysis
Date: August 2025

Standalone version with all modules included
"""
...
class TensileAnalysisGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Tensile Test Analysis Program - ASTM E8/E112")
    ...
    def plot_engineering_vs_true(self):
        ...
        ult_strain_true = true_strain[ult_idx]
        ult_stress_true = true_stress[ult_idx]
    ...
    def plot_ramberg_osgood(self):
        ...
        # Highlight the region used for fitting (post-yield)
        ax.axvline(x=yield_strain, color='gray', linestyle=':', alpha=0.7, 
                  label=f'Yield Point (fitting starts here)')
        ...
        if K > 0 and n > 0:
            # Create stress range for fitting - focus on the fitted region
            max_stress = np.max(stress)
            stress_fit = np.linspace(yield_stress * 0.8, max_stress, 1000)
            ...
            ax.plot(strain_fit, stress_fit, 'r--', linewidth=3, alpha=0.8,
                   label=f'Ramberg-Osgood Fit (R²={r_squared:.3f})')
            
            # Also plot elastic line for comparison
            elastic_strain_line = stress_fit / E
            ax.plot(elastic_strain_line, stress_fit, 'g:', linewidth=2, alpha=0.6,
                   label='Elastic Component (σ/E)')
            
            # Plot plastic component separately if visible
            if np.max(plastic_strain_fit) > 0.001:
                plastic_total = elastic_strain_line + plastic_strain_fit
                ax.plot(plastic_total - elastic_strain_line, stress_fit, 'm:', linewidth=2, alpha=0.6,
                       label=f'Plastic Component (σ/K)ⁿ')
        
        # Add data points at key locations for better visualization
        ax.plot(yield_strain, yield_stress, 'ro', markersize=8, 
               label=f'Yield Point ({yield_stress:.0f} MPa)')
        ...
        # Add enhanced parameter text box with fit quality assessment
        if K > 0 and n > 0:
            fit_quality = "Excellent" if r_squared > 0.95 else "Good" if r_squared > 0.90 else "Fair" if r_squared > 0.80 else "Poor"
            textstr = f'Ramberg-Osgood Parameters:\nK = {K:.1f} MPa\nn = {n:.3f}\nR² = {r_squared:.4f} ({fit_quality})\n\nEquation: ε = σ/E + (σ/K)ⁿ'
    ...
    def plot_hollomon_equation(self):
        ...
        if K > 0 and n > 0:
            ...
            strain_fit = np.linspace(0.001, np.max(true_strain), 1000)  # Start from small positive value
        ...
        if K > 0 and n > 0:
            textstr = f'K = {K:.1f} MPa\nn = {n:.3f}\nR² = {hollomon_params["r_squared"]:.4f}'
...
Yield Strength (σᵧ) - 0.2% Offset Method:
• Draw line parallel to elastic portion with 0.2% strain offset
• σᵧ = stress at intersection with stress-strain curve

Ultimate Tensile Strength (UTS):
UTS = σₘₐₓ = Maximum stress in stress-strain curve

Fracture Strength (σf):
σf = Stress at final data point (fracture)

Uniform Elongation:
Uniform Elongation (%) = εᵤₜₛ × 100
Where: εᵤₜₛ = Engineering strain at ultimate tensile strength
• Represents the elongation at maximum load before necking begins
• Important indicator of material ductility and formability

Elastic Modulus (E) - Secant Method:
E = Δσ / Δε (in middle 50% of elastic region)
• Calculated using linear regression: σ = E × ε
...
Uniform Elongation (%) = εᵤₜₛ × 100
Where: εᵤₜₛ = Engineering strain at ultimate tensile strength
• Represents the elongation at maximum stress before necking begins
• Important indicator of material ductility and formability

Elongation at Fracture:
Elongation (%) = εf × 100
Where: εf = Engineering strain at maximum strain (final data point)
• Represents the total elongation at specimen fracture
...
RAMBERG-OSGOOD EQUATION (VARIANT):
...
• First term (σ/E): Elastic strain component
• Second term (σ/K)ⁿ: Plastic strain component
• K: Strength coefficient related to yield strength (K ≈ 0.5-4 × yield strength)
• n: Controls rate of strain hardening (n > 0, can be > 1)
  - n < 1: Decreasing strain hardening rate (typical for metals)
  - n = 1: Linear strain hardening
  - n > 1: Increasing strain hardening rate (some polymers, composites)

Fitting Method:
• Enhanced multi-start optimization with 12 initial parameter sets
• Parameter bounds: K ∈ (0.01×σmax, 10×σmax), 0.01 < n < 10.0
• Initial guesses based on yield strength estimates
• Applied to plastic portion of stress-strain curve (post-yield)
• Weighted residuals emphasizing higher stress regions
• Multiple optimization methods (L-BFGS-B, TNC)
• Differential evolution with 4 strategies and penalty functions
• Basin-hopping optimization for challenging cases
• Advanced overflow protection for extreme parameter values
• R² value indicates goodness of fit quality assessment
...
Fitting Method:
• Linear regression in logarithmic space: ln(σₜ) vs ln(εₜ)
• Applied to plastic portion of true stress-strain curve
• Slope = n (strain hardening exponent)
• Intercept = ln(K), therefore K = exp(intercept)
• R² calculated in original stress-strain space

Physical Interpretation:
• K: Strength coefficient representing material strength
• n: Strain hardening exponent (0 < n < 1)
  - Higher n: greater strain hardening capability
  - Lower n: material approaches perfect plasticity
...
• Ramberg-Osgood equation useful for engineering stress-strain data"""
    ...
    def show_about(self):
        """Show about dialog"""
        about_text = """Tensile Test Analysis Program
        
Version: 1.0
Compliance: ASTM E8, ASTM E112

This program analyzes tensile test data to determine:
• Yield strength (0.2% offset method)
• Ultimate tensile strength
• Elastic modulus (middle 50% of elastic region)
